...
export const createService = async (serviceData, landlordId) => {
  ...
  // Validate price using validation helper
  validatePrice(price, 'service_price');

  const connection = await pool.getConnection();
  
  try {
    // Check if service name already exists for this landlord
    const [existing] = await connection.query(
      'SELECT id FROM services WHERE name = ? AND landlord_id = ?',
      [name, landlordId]
    );
...
export const getServiceById = async (serviceId, landlordId) => {
  ...
  try {
    const [rows] = await connection.query(
      'SELECT * FROM services WHERE id = ? AND landlord_id = ?',
      [serviceId, landlordId]
    );
...
export const deleteService = async (serviceId, landlordId) => {
  ...
  try {
    ...
    // Delete room_services associations first (cascade delete)
    await connection.query(
      'DELETE FROM room_services WHERE service_id = ?',
      [serviceId]
    );

    // Delete service record second
    await connection.query(
...
export const assignServiceToRoom = async (roomId, serviceId, landlordId, customPrice) => {
  ...
  try {
    ...
    // Insert room_service
    await connection.query(
      'INSERT INTO room_services (room_id, service_id, custom_price) VALUES (?, ?, ?)',
      [roomId, serviceId, price]
    );
...
export const updateRoomServicePrice = async (roomId, serviceId, landlordId, newPrice) => {
  ...
  try {
    ...
    // Update custom price
    await connection.query(
      'UPDATE room_services SET custom_price = ? WHERE room_id = ? AND service_id = ?',
      [newPrice, roomId, serviceId]
    );
