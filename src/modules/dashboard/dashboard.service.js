...
export const getTenantDashboard = async (tenantUserId) => {
  ...
  try {
    const [tenantInfo] = await connection.query('SELECT * FROM tenant WHERE user_id = ?', [tenantUserId]);
    ...
    if (activeContract) {
      const [invoiceInfo] = await connection.query('SELECT * FROM invoices WHERE contract_id = ? AND month = ? AND year = ?', [
        activeContract.id,
        currentMonth,
        currentYear,
      ]);
