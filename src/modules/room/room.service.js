...
export const createRoom = async (roomData, ownerId) => {
  try {
    const { room_number, floor, area, price, status, description, deposit } = roomData;
    ...
    const [result] = await connection.query(
      ...
      [room_number, floor || null, area || null, price, status || 'available', description || null, ownerId]
    );
...
export const updateRoom = async (roomId, roomData, ownerId) => {
  try {
    ...
    if (deposit !== undefined) validatePrice(deposit);
    ...
    const [existing] = await connection.query(`SELECT * FROM rooms WHERE id = ? AND owner_id = ?`, [roomId, ownerId]);
    if (existing.length === 0) {
      connection.release();
...
export const deleteRoom = async (roomId, ownerId) => {
  try {
    ...
    await connection.query(`DELETE FROM rooms WHERE id = ? AND owner_id = ?`, [roomId, ownerId]);
    connection.release();
