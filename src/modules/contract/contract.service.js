...
import { validateNotPastDate } from '../../common/utils/validators.js';
...
export const createContract = async (contractData, landlordUserId) => {
  ...
  try {
    ...
    if (tenantCheck.length === 0) throw new Error('Người thuê không tồn tại');
    if (!tenantCheck[0].is_active) throw new Error('Người thuê đã bị vô hiệu hóa');
    ...
    if (roomCheck.length === 0) throw new Error('Phòng không tồn tại hoặc không thuộc về bạn');
    if (roomCheck[0].status !== 'available') throw new Error('Phòng không trống, hiện đang được thuê');

    const [tenantContracts] = await connection.query(
      'SELECT id FROM contracts WHERE tenant_id = ? AND status = "active" FOR UPDATE',
      [tenant_id]
    );
    if (tenantContracts.length > 0) throw new Error('Người thuê này đã có phòng');

    const [roomContracts] = await connection.query(
      'SELECT id FROM contracts WHERE room_id = ? AND status = "active" FOR UPDATE',
      [room_id]
    );
    if (roomContracts.length > 0) throw new Error('Phòng đã có người thuê');

    if (contract_code) {
      const [codeCheck] = await connection.query('SELECT id FROM contracts WHERE contract_code = ?', [contract_code]);
      if (codeCheck.length > 0) throw new Error('Mã hợp đồng đã tồn tại');
    }
    ...
    await connection.query('UPDATE rooms SET status = "rented" WHERE id = ?', [room_id]);

    await connection.commit();

    const [newContract] = await connection.query('SELECT * FROM contracts WHERE id = ?', [contractId]);
...
export const getContractById = async (contractId, landlordUserId) => {
  ...
  try {
    ...
    if (rows.length === 0) throw new Error('Không tìm thấy hợp đồng');
    return rows[0];
  } finally {
    connection.release();
  }
...
export const terminateContract = async (contractId, landlordUserId) => {
  ...
  try {
    ...
    if (contractCheck.length === 0) throw new Error('Không tìm thấy hợp đồng');
    if (contractCheck[0].owner_id !== landlordUserId) throw new Error('Bạn không có quyền kết thúc hợp đồng này');
    if (contractCheck[0].status === 'terminated') throw new Error('Hợp đồng đã được kết thúc trước đó');

    await connection.query(`UPDATE contracts SET status = 'terminated' WHERE id = ?`, [contractId]);
    await connection.query('UPDATE rooms SET status = "available" WHERE id = ?', [contractCheck[0].room_id]);

    await connection.commit();

    return { id: contractId, status: 'terminated' };
  } catch (error) {
    await connection.rollback();
    throw error;
  } finally {
...
export const getActiveContractsByRoom = async (roomId) => {
  ...
  try {
    const [rows] = await connection.query('SELECT * FROM contracts WHERE room_id = ? AND status = "active"', [roomId]);
    return rows;
  } finally {
    connection.release();
  }
