...
export const markAsRead = async (notificationId, userId) => {
  ...
  try {
    const [existing] = await connection.query('SELECT * FROM notifications WHERE id = ? AND user_id = ?', [
      notificationId,
      userId,
    ]);
...
export const markAllAsRead = async (userId) => {
  ...
  try {
    await connection.query('UPDATE notifications SET is_read = TRUE WHERE user_id = ? AND is_read = FALSE', [userId]);
...
export const deleteNotification = async (notificationId, userId) => {
  ...
  try {
    await connection.query('DELETE FROM notifications WHERE id = ? AND user_id = ?', [notificationId, userId]);
